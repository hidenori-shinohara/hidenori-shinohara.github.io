...
# Summary
FHE is a cryptographic scheme that allows addition and multiplication of data without decrypting.
FHE represents messages and cipher texts as vectors in a lattice.
Clever uses of vector and matrix operations and modular arithmetics enable encryption and decryption.
Key generation in FHE is based on the closest vector problem, which is considered to be hard.
Addition of cipher texts are done by adding the corresponding vectors.
Multiplication is also possible but less straightforward.

# Choices of parameters $p, q, n$

1. FHE's considered secure since CVP is considered hard.
2. One of the most efficient ways to solve CVP fails if the dimension is too high.
3. Therefore, if FHE uses a high dimension, it's considered impossible to break.
4. You should pick sufficiently large $n$.
...
- $A$ is the basis of the lattice in question.
  More specifically, the set of the column vectors in $A$ forms a basis.
  I need to look this up more carefully but there must be a theorem stating that $A$ is statistically guaranteed to be invertible for a big $q$ and $n$.
- $e$ is the small error that we're introducing.
  We multiply it by $p$, so we can take modulo $p$ later to remove the error.
  Note that $p \ll q$ and $e$ is drawn from the normal distribution with an appropriate variance, so $pe$ is "small".
- $s$ is the private key that we don't publish.
- Note that this is exactly the closest vector problem (CVP)
  $As$ is a random vector in the lattice since $s$ is specifies the linear combination of the basis.
  $pe$ is a random error vector that we add.
  $As$ is the closest vector in the lattice to $As + pe$.
  so finding $As$ from $As + pe$ is an instance of the closest vector problem.
  As we assume $A$ is invertible, finding $s$ is equivalent to finding $As$.
  So finding the private key is as difficult as the closest vector problem.
...
Notes:
- As we assume $A$ is invertible, it is easy to find $r_1$.
  Therefore, I believe $r_1$ isn't meant to be a private key.
  I think it just introduces randomness to the encryption.
- I need to think a lot more about this, but it looks like only the first entry of $b_1$ matters.
  The remaining entry in $b_1$ is just garbage produced by $pe_1$.
- The error vectors $e$ and $e_1$ are multiplied by $p$.
  This lets us remove $p$ by simply taking mod $p$.
...
- It looks like what happens is:
  - Create a lattice with $A \in \mathbb{Z}_q^{n \times n}$.
  - Choose a secret, random vector by picking $s \in \mathbb{Z}_q^{n}$.
  - The final encryption is a summation of $rAs$, the message and an error vector.
  - Both parties work together to create $r A s$ where $r$ is a row vector supplied upon encryptiona nd $s$ is a column vector supplied upon key generation.
    The scheme cleverly allows one to add and subtract $rAs$ once, hence cancelling it out.
  - All the error vectors are multiplied by $p$, so taking modulo $p$ erases the errors.
...
The article illustrates the idea, but the math seems somewhat inaccurate and incomplete.

First, it says $b_1 − u_1a_1^T s = pe + m_1$, but this is incorrect.
$b_1 - u_1a_1^T s \equiv u_1pe^Tr_1 + pe_1 + m_1u_1 \pmod q$.
For this purpose, we will use $E$ to denote the error term, so we will write $b_1 - u_1a_1^Ts = pE + m_1$.

Let $c_1 = (a_1, b_1)$ and $c_2 = (a_2, b_2)$ be given.
Then let $f_1(x) = b_1 - u_1 a_1^T x$ and $f_2(x) = b_2 - u_1 a_2^T x$.
$f_1(s_1) = pE_1 + m_1$, $f_2(s_1) = pE_2 + m_2$ where $E_1, E_2$ are small error terms.
By taking modulo $p$, we obtain $f_1(s_1) \cong m_1$ and $f_2(s_1) \cong m_2$.
Therefore, by multiplying $f_1(x)$ by $f_2(x)$, we obtain a function that takes $s_1$ and returns a value that equals $m_1 m_2$ upon decryption.
The blog does not explain how that will be represented.
However, it is easy to see that $f_1 \cdot f_2$ will contain one coefficient for $s_i s_j$ for every $i, j$.
Therefore, $f_1f_2$ contains $O(n^2)$ summands and thus the cipher text's size increases quadratically after a multiplication.

Finally, $f_1(s_1)f_2(s_2)$'s leading error term is $p^2 E_1 E_2$.
Assuming $E_1$ and $E_2$ are significantly smaller than $p$, we just need to ensure $p^2 < q / 2$.
It is easy to see that we can't perform more than $\log_{p}{q}$ multiplications.
...
I don't fully understand this concept, but here's what I've understood.

1. You have a message $m_1$ and a private key $s_1$.
   Encrypt $m_1$ with $s_1$.
   We will call the cyphertext $c_1$.
1. You create a new private key $s_2$.
   We do two things:
   1. Encrypt $c_1$ with $s_2$. Call it $c_1'$.
   1. Encrypt $s_1$ with $s_2$. Call it $s_1'$.
1. Decrypt $c_1'$ with $s_1'$.
   Call it $c_2$.

Claim: If you decrypt $c_2$ with $s_2$, you get $m_1$.
In other words, it's as if you simply encrypted $m_1$ with $s_2$ without actually encrypting it.

Let $d, e$ denote the decryption and encryption functions.

$$
\begin{align*}
  d(c_2, s_2)
    &= d(d(c_1', s_1'), s_2) \\
    &= d(d(e(c_1, s_2), e(s_1, s_2)), s_2) \\
    &? d(c_1, s_1) \\
    &= m_1.
\end{align*}
$$
...
Also, I'm not sure how this is helpful.

FHE seems to involve at least two players:
One is the owner of the data and the other is the operator of the encrypted data.
Only the owner can perform bootsrapping.
However, if the owner can also simply decrypt the data and re-encrypt it.
That seems much simpler.

# Questions
- How does one define how orthogonal and short a basis is?
- I'm not familiar with the concept of a norm in $\mathbb{Z}_q^k$.
  Does modular arithmetic affect the norm calculation?
- The article says:
> For our scheme we draw our error randomly according to a normal law centered at 0 with sufficiently high variance, hence \$sigma$.
- However, this seems like a bad idea.
  The error should never be 0.
