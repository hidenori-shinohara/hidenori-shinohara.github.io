...
- Note that this is exactly the closest vector problem (CVP)
  $As$ is a random vector.
  $pe$ is a random error vector that we add.
  $As$ is the closest vector to $As + pe$ in the lattice,
  so finding $As$ from $As + pe$ is an instance of the closest vector problem.
  I need to look this up more carefully but I believe that $A$ is statistically guaranteed to be invertible for a big $q$ and $n$.
  If that's the case, finding $s$ is equivalent to finding $As$.
